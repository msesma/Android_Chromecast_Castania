...
public class MainActivity extends ActionBarActivity {
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		// devices
		mSelector = new MediaRouteSelector.Builder().addControlCategory(CastMediaControlIntent.categoryForCast(APP_ID)).build();
				...
				public void onStatusUpdated() {
					...
					if (mediaStatus != null) {
						...
						case MediaStatus.PLAYER_STATE_PLAYING:
							mIsPlaying = true;
							((ImageButton) findViewById(R.id.bt_play)).setImageDrawable(getResources().getDrawable(R.drawable.pause));
							break;
						default:
							mIsPlaying = false;
							((ImageButton) findViewById(R.id.bt_play)).setImageDrawable(getResources().getDrawable(R.drawable.play));
						}
						((LinearLayout) findViewById(R.id.ll_media_buttons)).setVisibility(View.VISIBLE);
					} else {
						((LinearLayout) findViewById(R.id.ll_media_buttons)).setVisibility(View.INVISIBLE);
					}
				}
	...
	private void loadMedia() {
		...
		MediaInfo mediaInfo;
		if (photoMode) {
			mediaInfo = new MediaInfo.Builder(url).setContentType("image/jpeg").setStreamType(MediaInfo.STREAM_TYPE_NONE).setMetadata(mediaMetadata).build();
		} else {
			mediaInfo = new MediaInfo.Builder(url).setContentType("video/mp4").setStreamType(MediaInfo.STREAM_TYPE_BUFFERED).setMetadata(mediaMetadata).build();
		}
		try {
